...
class tenpay{
	 ...
	 * @var string
	 */
	private $gateway   = 'http://service.tenpay.com/cgi-bin/v3.0/payservice.cgi';
	/**
	 * 支付接口标识
	 *
	 * @var string
	...
	public function get_payurl(){
		...
		/* 商户号 */
		$bargainor_id = $this->payment['payment_config']['tenpay_account'];//"1215486001";
		
		/* 密钥 */
		$key = $this->payment['payment_config']['tenpay_key'];//"admin15805993170admin15805993170";
		...
		$reqHandler->setParameter("desc", $transaction_id);	//商品名称
		
		$modeltype = intval($this->order['modeltype']);//模块类型 1表示金币充值，2表示积分兑换
        if ($modeltype > 0 ){
			$reqHandler->setParameter("attach","$modeltype");	//自定义参数
        }else {
        	$reqHandler->setParameter("attach", '1');	//自定义参数
        }
