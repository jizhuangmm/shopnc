      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td>
          	...
          	&nbsp;
            <?php if((!empty($v['store_image']) && $v['name_auth'] == 0) || (!empty($v['store_image1']) && $v['store_auth'] == 0)){?>
            (<?php echo $lang['authing'];?>)
            <?php }?>
            <?php echo $v['store_cs_is_open'] == 2 ? '<span style="color:red;"> </span>' : '' ;?></td>
		  ...
		  <td class="align-center yes-onoff">&nbsp;</td>
          <td class="align-center yes-onoff"><?php if($v['store_recommend'] == '0'){ ?>
            <a href="JavaScript:void(0);" class="tooltip disabled" ajax_branch='store_recommend' nc_type="inline_edit" fieldname="store_recommend" fieldid="<?php echo $v['store_id']?>" fieldvalue="0" title="<?php echo $lang['editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
            <?php }else { ?>
            <a href="JavaScript:void(0);" class="tooltip enabled" ajax_branch='store_recommend' nc_type="inline_edit" fieldname="store_recommend" fieldid="<?php echo $v['store_id']?>" fieldvalue="1" title="<?php echo $lang['editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
            <?php } ?></td>
          <td class="align-center">
		   <a href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['nc_edit'];?></a> | <a href="javascript:;" onclick="if(confirm('<?php echo $lang['are_you_sure_del_store'];?>')){location.href='index.php?act=store&op=store_del&store_id=<?php echo $v['store_id']?>';}"><?php echo $lang['nc_del'];?></a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
