...
<div class="wrap">
  <div class="tabmenu">
	...
	foreach ($output['member_menu'] as $key => $val) {
		...
		}elseif ($val['menu_key'] == 'seller'){
			echo '<li class="'.$classname.'"><a href="'.$val['menu_url'].'">'.$val['menu_name'].'(<span style="color: red;">'.$output['newseller'].'</span>)</a></li>';
		}else{
  ...
  <div class="ncu-form-style">
    ...
    <dl>
      <dt>
        ...
        <a href="index.php?act=show_store&id=<?php echo $v['store_id'];?>" target="_blank"><?php echo $v['from_member_name']; ?></a> <?php echo $lang['home_message_speak']; }elseif ($output['drop_type'] == 'msg_system') { 
        	echo $v['from_member_name']; 
        	} else { 
        		echo $v['from_member_name'].$lang['home_message_speak'];} ?><?php echo $lang['nc_colon'];?></dt>
      <dd>
        <p><?php echo nl2br($v['message_body']); ?></p>
