...
<div id="lazymodule">
  ...
  <ul class="fd-list">
    ...
    <li nc_type="tracerow_<?php echo $v['trace_id']; ?>">
      <div class="fd-aside"><span class="thumb size60"><i></i> <a href="index.php?act=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank"> <img src="<?php if ($v['trace_memberavatar']!='') { echo ATTACH_AVATAR.DS.$v['trace_memberavatar']; } else {  echo ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" onload="javascript:DrawImage(this,60,60);"> </a> </span> 
        <!--<p class="arrow"></p>--> 
      </div>
      <dl class="fd-wrap">
        <dt>
          <h3><a href="index.php?act=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank"><?php echo $v['trace_membername'];?></a><?php echo $lang['nc_colon'];?></h3>
        ...
        <dd>
          <?php if ($v['trace_originalid'] > 0){//转帖内容?>
          <div class="fd-forward">
            ...
            <div class="stat"><span><?php echo $lang['sns_forward']; ?><?php echo $v['trace_orgcopycount']>0?"({$v['trace_orgcopycount']})":''; ?></span>&nbsp;&nbsp; <span><a href="index.php?act=member_snshome&op=traceinfo&mid=<?php echo $v['trace_originalmemberid'];?>&id=<?php echo $v['trace_originalid'];?>" target="_blank"><?php echo $lang['sns_comment']; ?><?php echo $v['trace_orgcommentcount']>0?"({$v['trace_orgcommentcount']})":''; ?></a></span> </div>
            <?php }?>
          </div>
          <?php } else {?>
          <?php echo parsesmiles($v['trace_content']);?>
        ...
        <dd> <span class="goods-time fl"><?php echo date('Y-m-d H:i',$v['trace_addtime']);?></span> <span class="fr"><a href="javascript:void(0);" nc_type="fd_forwardbtn" data-param='{"txtid":"<?php echo $v['trace_id'];?>"}'><?php echo $lang['sns_forward']; ?></a>&nbsp;|&nbsp;<a href="javascript:void(0);" nc_type="fd_commentbtn" data-param='{"txtid":"<?php echo $v['trace_id'];?>","mid":"<?php echo $v['trace_memberid'];?>"}'><?php echo $lang['sns_comment']; ?><?php echo $v['trace_commentcount']>0?"({$v['trace_commentcount']})":'';?></a></span> </dd>
        <!-- 评论模块start -->
