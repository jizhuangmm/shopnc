...
<div class="feededitor">
  <form method="post" action="index.php?act=member_snsindex&op=sharegoods&irefresh=1" id="sharegoods_form">
    <input type="hidden" name="form_submit" value="ok"/>
    <!-- 商品图片start -->
    <?php if (!empty($output['goods_list'])){?>
    <ul id="mycarousel" class="snsgoodsimglist jcarousel-skin-tango">
      <?php foreach ($output['goods_list'] as $v) {?>
      <li><a href="javascript:void(0);" value="<?php echo $v['goods_id'] ?>"> <span class="thumb size90"><i></i> <img title="<?php echo $v['goods_name']?>" src="<?php echo thumb($v,'small');?>" onload="javascript:DrawImage(this,90,90);"/>
