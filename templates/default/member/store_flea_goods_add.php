...
background: url(<?php echo TEMPLATES_PATH;
?>/images/member/btn.gif) repeat 0 -1018px;
	float: right;
}
.dialog_body {
	border: 0px;
...
.flea_pic_list ul.menu {
	background-color: #FFF;
	width: 648px;
	height: 32px;
	border-bottom: solid 1px #A7CAED;
}
.flea_pic_list .menu li {
	float: left;
	cursor: pointer;
}
.flea_pic_list .menu li a {
	font-size: 12px;
	line-height: 20px;
	color: #555;
	height: 20px;
...
.flea_pic_list .menu li.active a, .goods-pictures .menu li.active a:hover {
	font-size: 13px;
	font-weight: 600;
	color: #36C;
	background-color: #FFF;
...
.flea_pic_list .content {
	width: 648px;
	background-color: #FFF;
	border: solid 1px #A7CAED;
	border-top: 0;
}
.flea_pic_list .content .standard {
	padding: 10px 5px 10px 5px;
*padding-bottom : 5px;
}
.flea_pic_list .content .standard .big_pic {
	background-color: #FFF;
	margin-left: 7px;
	display: inline;
	width: 190px;
...
.flea_pic_list .content .standard .big_pic .picture { /* if IE7/8/9*/
*text-align: center;
	display: inline;
	width: 182px;
	height: 182px;
...
.flea_pic_list .content .standard .small_pic {
	width: 430px;
	float: left;
	margin-left: 7px;
	display: inline;
...
.flea_pic_list .content .standard .small_pic ul {
	width: 450px;
	float: left;
	display: inline;
	padding-bottom: 8px;
}
.flea_pic_list .content .standard .small_pic ul li {
	display: inline;
	width: 80px;
	height: 114px;
	float: left;
...
.flea_pic_list .content .standard .small_pic .picture {
...
.flea_pic_list .content .standard .small_pic .bg {
	display: none;
	width: 70px;
	height: 70px;
...
.flea_pic_list .content .standard .small_pic .operation {
...
.flea_pic_list .content .standard .small_pic .cut_in {
background: url(<?php echo TEMPLATES_PATH;
?>/images/member/picedit.png) 0px -64px;
	width: 16px;
	height: 16px;
	float: left;
	margin-left: 7px;
...
.flea_pic_list .content .standard .small_pic .delete {
background: url(<?php echo TEMPLATES_PATH;
?>/images/member/picedit.png) 0 0;
	display: inline;
	width: 16px;
	height: 16px;
	float: right;
...
.flea_pic_list .content .standard .small_pic .upload-btn {
	width: 74px;
	height: 32px;
	padding: 0px;
	position: absolute;
...
.flea_pic_list .content .standard .small_pic .titles {
	width: 66px;
	height: 16px;
	line-height: 15px;
	text-align: center;
...
.flea_pic_list .content .standard .small_pic .line {
	width: 66px;
	height: 2px;
	overflow: hidden;
	background: #d8deda;
...
.flea_pic_list .content .standard .small_pic .help {
	line-height: 18px;
...
.flea_pic_list .standard .small_pic .help p {
	padding-left: 24px;
	width: 398px;
}
.flea_pic_list .content .standard .help .ico {
background: url(<?php echo TEMPLATES_PATH;
?>/images/member/ico.gif) no-repeat 3px -2867px;
	color: #F60;
}
.flea_goods-attribute {
	width: 792px;
	margin-top: 0;
	border: none;
}
.flea_goods-attribute dl dd p {
	float: none;
}
.goods-gallery {
	width: 650px;
  ...
  <div class="ncu-form-style">
    <form method="post" id="goods_form" action="index.php?act=member_flea&op=<?php if ($output['goods']['goods_id']!='') echo 'edit_save_goods'; else echo 'save_goods'; ?>">
      ...
      <dl>
        ...
        <dd>
          <p>
            <input type="text" class="text" name="goods_name" value="<?php echo $output['goods']['goods_name']; ?>"  />
          </p>
      ...
      <dl>
        ...
        <dd>
          <p>
            <input type="text" class="text" name="goods_tag" value="<?php echo $output['goods']['goods_tag']; ?>"/>
          </p>
              ...
              <div class="standard">
                <div id="goodsCoverPicture" class="big_pic">
                  <div class="picture"> <span class="thumb size182"><i></i>
                    ...
                    <?php } else { ?>
                    <img id="big_goods_image" src="<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image']; ?>" onload="javascript:DrawImage(this,182,182);" alt="" />
                    <?php } ?>
                    </span> </div>
                </div>
                <div class="small_pic" >
                  <ul id="goods_images">
                    ...
                    <li nc_type="handle_pic" file_id="<?php echo $output['goods_image'][$i]['upload_id']; ?>" thumbnail="<?php echo $output['goods_image'][$i]['file_thumb']; ?>" id="thumbnail_<?php echo $i;?>">
                      ...
                      <div class="picture"><span class="thumb size60"><i></i><img id="img_<?php echo $i;?>" src="<?php echo $output['goods_image'][$i]['file_thumb'] !=''?$output['goods_image'][$i]['file_thumb']:ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image'];?>" onerror="this.src='<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image'];?>'" onload="javascript:DrawImage(this,60,60);"/></span></div>
      ...
      <dl>
        ...
        <dd >
          <p>
            <input type="text" class="text" name="seo_keywords" value="<?php echo $output['goods']['goods_keywords']; ?>" />
          </p>
      ...
      <dl>
        ...
        <dd>
          <p>
            <textarea name="seo_description" rows="3" id="remark_input" ><?php echo $output['goods']['goods_description']; ?></textarea>
          </p>
      ...
      <dl>
        ...
        <dd>
          <p>
            <?php showEditor('goods_body',$output['goods']['goods_body'],'658px','400px','visibility:hidden;',"false",$output['editor_multimedia']);?>
          </p>
...
<script type="text/javascript" charset="utf-8" src="<?php echo RESOURCE_PATH;?>/js/common_flea_select.js"></script> 
<script type="text/javascript" charset="utf-8" src="<?php echo RESOURCE_PATH;?>/js/store_flea_goods.js"></script> 
<script type="text/javascript">
...
function add_uploadedfile(file_data)
{
    ...
    if(file_data.instance == 'goods_image'){
        ...
        if($('#big_goods_image').attr('src') == '<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image']; ?>'){
    ...
    if (confirm('<?php echo $lang['store_goods_index_goods_del_confirm']; ?>')){
            ...
            $.getJSON(url, {'id':goods_file_id}, function(data){
                if (data.done)
                {
                	...
                	$('#img_'+id).attr('src','<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image']; ?>');
                	$('#'+id).html('');
                }
                else
                {
            ...
            if($('#big_goods_image').attr('src') == $('#img_'+id).attr('src')){
				$('#big_goods_image').attr('src','<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image']; ?>');
            }
        }
}
