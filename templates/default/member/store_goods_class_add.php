<div class="eject_con">
  <div id="warning"></div>
  <form id="category_form" method="post" target="_parent" action="index.php?act=store&op=goods_class_save">
    <?php if ($output['class_info']['stc_id']!=0) { ?>
    <input type="hidden" name="stc_id" value="<?php echo $output['class_info']['stc_id']; ?>" />
    <?php } ?>
    <dl>
      <dt class="required"><em class="pngFix"></em><?php echo $lang['store_goods_class_name'].$lang['nc_colon'];?></dt>
      <dd>
        <input class="w200 text" type="text" name="stc_name" id="stc_name" value="<?php echo $output['class_info']['stc_name']; ?>" />
      </dd>
    </dl>
    <?php if ($output['class_info']['stc_id']==0) { ?>
    <dl>
    ...
    <dl>
      ...
      <dd>
        <input class="w50 text" type="text" name="stc_sort" value="<?php echo intval($output['class_info']['stc_sort']); ?>"  />
      </dd>
    </dl>
    <dl>
      ...
      <dd>
        <label>
          <input type="radio" name="stc_state" value="1" <?php if ($_GET['stc_id']=='' or $output['class_info']['stc_state']==1) echo 'checked="checked"'; ?> />
          <?php echo $lang['store_create_yes'];?></label>
        <label>
          <input type="radio" name="stc_state" value="0" <?php if ($output['class_info']['stc_state']==0 and $_GET['stc_id']!='') echo 'checked="checked"'; ?> />
...
$(function(){
    $('#category_form').validate({
        ...
        messages : {
            stc_name : {
                required : '<?php echo $lang['store_goods_class_name_null'];?>'

            },
            stc_sort  : {
                number   : '<?php echo $lang['store_goods_class_input_int'];?>'
            }
        }
    });
});
