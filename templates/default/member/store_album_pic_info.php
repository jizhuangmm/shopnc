...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo SiteUrl; ?>/index.php?act=store_album&op=ajax_change_imgmessage", {'url':url}, function(data){
		...
		$("#upload_time").html('<b><?php echo $lang['album_pinfo_upload_time'];?>: </b>'+data.upload_time);
		$("#default_size").html('<b><?php echo $lang['album_pinfo_original_size'];?>: </b>'+data.default_size+'KB');
		$("#default_spec").html('<b><?php echo $lang['album_pinfo_original_spec'];?>: </b>'+data.default_spec);
	});
}
</script>