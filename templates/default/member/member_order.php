...
<div class="wrap">
    ...
    <table class="search-form">
      ...
      <tr>
        ...
        <td class="w100"><select name="state_type">
            ...
            <option value="order_submit" <?php echo $_GET['state_type']=='order_submit'?'selected':''; ?>><?php echo $lang['member_order_submited'];?></option>
  ...
  <table class="order ncu-table-style">
    ...
    <tbody>
      ...
      <tr>
        <td class="bdl"></td>
        <td><div class="goods-pic-small"><span class="thumb size60"><i></i><a href="?act=goods&goods_id=<?php echo $v['goods_id'];?>&id=<?php echo $v['store_id'];?>" target="_blank"><img src="<?php echo thumb($v,'tiny'); ?>" onload="javascript:DrawImage(this,60,60);" /></a></span></div></td>
        <td><dl class="goods-name">
            <dt><a href="?act=goods&goods_id=<?php echo $v['goods_id'];?>&id=<?php echo $v['store_id'];?>" target="_blank"><?php echo $v['goods_name']; ?></a></dt>
        ...
        <td class="bdl" rowspan="<?php echo count($val);?>"><?php 
			...
			if(intval($val[0]['order_state']) >= 20 && $time_limit) {
				?>
          <a href="index.php?act=member_complain&op=complain_submit&order_id=<?php echo $val[0]['order_id']; ?>" target="_blank" class="ncu-btn2"><?php echo $lang['member_order_complain'];?></a>
          <?php } ?></td>
        <td class="bdl" rowspan="<?php echo count($val);?>"><p class="store-name"><a href="index.php?act=show_store&id=<?php echo $val[0]['store_id']; ?>" target="_blank" title="<?php echo $val[0]['store_name']; ?>"><?php echo $val[0]['store_name']; ?></a></p>
          <p><a href="index.php?act=member_snshome&mid=<?php echo $val[0]['member_id'];?>" title="<?php echo $lang['member_evaluation_storekeeper'];?>"><?php echo $val[0]['member_name'];?></a>&nbsp;<a target="_blank" href="index.php?act=home&op=sendmsg&member_id=<?php echo $val[0]['seller_id']; ?>" class="message" title="<?php echo $lang['nc_message'];?>"></a></p><p>
          <?php if(!empty($val[0]['store_qq'])){?>
          <a href="http://wpa.qq.com/msgrd?V=1&amp;Uin=<?php echo $val[0]['store_qq'];?>&amp;Site=<?php echo $val[0]['store_qq'];?>&amp;Menu=yes" target="_blank" class="vm"><img src="http://wpa.qq.com/pa?p=1:<?php echo $val[0]['store_qq'];?>}:4" alt="QQ"></a>
          <?php }?>
          <?php if(!empty($val[0]['store_ww'])){?>
          <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $val[0]['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"  class="vm" ><img border="0" src="http://amos.im.alisoft.com/online.aw?v=2&uid=<?php echo $val[0]['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" /></a>
          <?php }?>
          <?php if(!empty($val[0]['store_msn'])){?>
          <a target="_blank" href="http://settings.messenger.live.com/Conversation/IMMe.aspx?invitee=<?php echo $val[0]['store_msn'];?>" class="vm"><img src="http://messenger.services.live.com/users/<?php echo $val[0]['store_msn'];?>/presenceimage/" alt="status" /></a>
          <?php }?></p></td>
        ...
        <td class="bdl bdr" rowspan="<?php echo count($val);?>"><p><?php echo $val[0]['state_info']; ?><br/>
          ...
          <?php } ?>
          <?php if ($val[0]['order_state'] == 40 && $val[0]['able_evaluate']) { ?>
