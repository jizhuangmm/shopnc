    ...
    <tbody>
      ...
      <tr>
        <th colspan="20"><span class="fl ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><span class="goods-num"><em><?php echo $val[0]['order_sn']; ?></em>
          ...
          </span> <span class="fl ml20"><?php echo $lang['store_order_add_time'].$lang['nc_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$val[0]['add_time']); ?></em></span> <span class="fl ml20"><a href="index.php?act=store&op=show_order&order_id=<?php echo $val[0]['order_id']; ?>" target="_blank" class="nc-show-order fl"><i></i><?php echo $lang['store_order_view_order'];?></a>
      ...
      <tr>
        <td class="bdl"></td>
        <td class="w70"><div class="goods-pic-small"><span class="thumb size60"><i></i><a href="?act=goods&goods_id=<?php echo $v['goods_id'];?>&id=<?php echo $v['store_id'];?>" target="_blank"><img src="<?php echo thumb($v,'tiny'); ?>" onload="javascript:DrawImage(this,60,60);" /></a></span></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a target="_blank" href="index.php?act=goods&goods_id=<?php echo $v['goods_id']; ?>&id=<?php echo $v['store_id'];?>"><?php echo $v['goods_name']; ?></a></dt>
        ...
        <td class="bdl" rowspan="<?php echo count($val);?>"><p>
            <?php if($v['refund_state']<2 and $v['order_state']>=20 and $v['order_state']<50) { ?>
            <a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_order_refund'];?>" dialog_id="seller_order_refund" dialog_width="480" uri="index.php?act=refund&op=add&order_id=<?php echo $v['order_id']; ?>" id="order<?php echo $v['order_id']; ?>_action_refund"><?php echo $lang['store_order_refund'];?></a>
            <?php } ?>
            <?php if($v['return_state']<2 and $v['order_state']>=40 and $v['order_state']<50) { ?>
            &nbsp;<a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_order_return'];?>" dialog_id="seller_order_return" dialog_width="480" uri="index.php?act=return&op=add&order_id=<?php echo $v['order_id']; ?>" id="order<?php echo $v['order_id']; ?>_action_return"><?php echo $lang['store_order_return'];?></a></p>
          <?php } ?>
        ...
        <td class="bdl" rowspan="<?php echo count($val);?>"><p><?php echo $v['buyer_name']; ?><a target="_blank" class="message" href="index.php?act=home&op=sendmsg&member_id=<?php echo $v['buyer_id'];?>"></a></p>
          <div class="buyer"><?php echo $output['order_array'][$v['order_id']]['true_name'];?>
            <p>
              ...
              <a href="http://wpa.qq.com/msgrd?V=1&amp;Uin=<?php echo $v['member_info']['member_qq'];?>&amp;Site=<?php echo $v['member_info']['member_qq'];?>&amp;Menu=yes" target="_blank"><img src="http://wpa.qq.com/pa?p=1:<?php echo $v['member_info']['member_qq'];?>}:4" alt="QQ"></a>
              <?php }?>
              <?php if(!empty($v['member_info']['member_ww'])){?>
              <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $v['member_info']['member_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" ><img border="0" src="http://amos.im.alisoft.com/online.aw?v=2&uid=<?php echo $val[0]['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" /></a>
              <?php }?>
              <?php if(!empty($v['member_info']['member_msn'])){?>
              <a target="_blank" href="http://settings.messenger.live.com/Conversation/IMMe.aspx?invitee=<?php echo $v['member_info']['member_msn'];?>"><img src="http://messenger.services.live.com/users/<?php echo $v['member_info']['member_msn'];?>/presenceimage/" alt="status" /></a>
              <?php }?>
            </p>
        ...
        <td class="bdl" rowspan="<?php echo count($val);?>"><p class="goods-price"><?php echo $v['order_amount']; ?></p>
          ...
          <p>
            ...
            <!--            <?php if ($v['shipping_time']!='' and $v['finnshed_time']=='') { ?>
            <a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_order_modify_no'];?>" uri="index.php?act=store&op=change_state&state_type=store_order_shipping&shipping_code=<?php echo $v['shipping_code']; ?>&order_sn=<?php echo $v['order_sn']; ?>&order_id=<?php echo $v['order_id']; ?>" dialog_id="seller_order_shipped" dialog_width="400" id="order<?php echo $v['order_id']; ?>_action_edit_invoice_no" /><?php echo $lang['store_order_modify_no'];?></a>
            <?php } ?>  --> 
          </p></td>
        <td class="bdl bdr" rowspan="<?php echo count($val);?>"><p><?php echo orderStateInfo($v['order_state']); ?>
          <?php if($val['evaluation_time']) { ?><br/><?php echo $lang['store_order_evaluated'];?>
          <?php } ?></p>
          ...
          <?php endif; ?>
          <?php if ($v['order_state'] == 40 && $v['able_evaluate']) { ?>
          ...
          <?php }?>
          <?php if (($v['payment_time'] != '' and $v['shipping_time']=='' and $v['order_state']>0 and $v['order_state']<40) or($v['order_state'] == 60)) { ?>
