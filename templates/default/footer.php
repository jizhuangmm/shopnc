...
//	if(!$.cookie(COOKIE_PRE+'scan')){
//		$('body').append('<iframe id="cronb_frame" style="display:none" src=""></iframe>');
//		$.cookie(COOKIE_PRE+'scan', unixtime, { path: '/', expires: date });
//		// 商品自动上下架
//		$('body').append('<iframe style="display:none" src="'+SITEURL+'/index.php?act=scan&type=updown"></iframe>');
//	}
	<?php if ( C('cron_product') == 1 && $_SESSION['store_id'] > 0){?>
	$.include('index.php?act=scan&type=updown');
	<?php }?>
//	if (!$.getCookie('cron_product'))) {$.include('index.php?act=scan&type=updown');}
	if ($.getCookie('cron_store') && !$.getCookie('ifcronstore')) {$.include('index.php?act=scan&type=store&id='+$.getCookie('cron_store'));}
});
</script>