...
final class flea_Cache {
    ...
    private static function getFlea_areaCache($param) {
        ...
        $lang = Language::getlangcontent();
        $deep = $param['deep'];
        $cache_file = BasePath . DS . "cache" . DS . "flea_area" . DS . "flea_area_" . $deep . ".php";
    ...
    public static function makeallcache($type) {
        ...
        switch ($type) {
            case "area" :
                $file_list = readfilelist(BasePath . DS . "cache" . DS . "flea_area");
                if (is_array($file_list)) {
                    foreach ($file_list as $v) {
                        @unlink(BasePath . DS . "cache" . DS . "flea_area" . DS . $v);
                    }
                }
                $maxdeep = 1;
            default :
                ...
                $result = Db::select($param);
                if (!empty($result)) {
                    $cache_file_area = BasePath . DS . "cache" . DS . "area" . DS . "flea_area_" . $maxdeep . ".php";
                    $tmp .= "<?php \r\n";
                    $tmp .= "defined('InShopNC') or exit('Access Invalid!'); \r\n";
                    $tmp .= "\$data = array(\r\n";
                    if (is_array($result)) {
