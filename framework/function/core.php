...
function showMessage( $msg, $url = "", $show_type = "html", $msg_type = "succ", $is_show = 1, $time = 2000 )
{
		...
		case "exception" :
				...
				echo "<dd><p /><p /><p /><p /></dd>";
				echo "<dd><p /><p /><p /><p />Copyright 2013- ShopNC, All Rights Reserved ".$lang['company_name']."</dd>";
				echo "</dl></body></html>";
				exit( );
		case "javascript" :
				echo "<script>";
...
function rcache( $key = NULL, $module = "", $prefix = "", $unserialize = TRUE )
{
		if ( empty( $key ) || c( "cache.type" ) == "file" || c( "memory.".$module.".open" ) === 0 )
		{
				return FALSE;
		}
		$obj_cache = Cache::getinstance( c( "cache.type" ) );
		$tmp_prefix = c( "memory.".$module.".prefix" );
		if ( trim( $tmp_prefix ) != "" )
		{
				$prefix = $tmp_prefix;
		}
...
function wcache( $key = NULL, $data = array( ), $module = "", $ttl = 0, $prefix = "", $serialize = TRUE )
{
		if ( empty( $key ) || c( "cache.type" ) == "file" || c( "memory.".$module.".open" ) === 0 )
		{
				return FALSE;
		...
		$tmp_ttl = c( "memory.".$module.".ttl" );
		if ( is_numeric( $tmp_ttl ) && 0 <= $tmp_ttl )
		{
				$ttl = $tmp_ttl;
		}
		$tmp_prefix = c( "memory.".$module.".prefix" );
...
{
		require_once( BasePath."/framework/libraries/json.php" );
		function json_encode( $code )
		{
				return Services_JSON::encode( $code );
		}
		function json_decode( $code, $assoc = FALSE )
		{
				return Services_JSON::decode( $code, $assoc ? 16 : 0 );
		}
}
?>
