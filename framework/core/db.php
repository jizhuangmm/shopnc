...
class ModelDb
{
		...
		{
				$sql = str_replace( array( "%TABLE%", "%DISTINCT%", "%FIELD%", "%JOIN%", "%WHERE%", "%GROUP%", "%HAVING%", "%ORDER%", "%LIMIT%", "%UNION%" ), array(
		...
		{
				$sql = "DELETE ".$this->parsePriority( $options )." FROM ".$this->parseTable( $options ).$this->parseWhere( isset( $options['where'] ) ? $options['where'] : "" ).$this->parseOrder( isset( $options['order'] ) ? $options['order'] : "" ).$this->parseLimit( isset( $options['limit'] ) ? $options['limit'] : "" );
		...
		{
				$sql = "UPDATE ".$this->parsePriority( $options ).$this->parseTable( $options ).$this->parseSet( $data ).$this->parseWhere( isset( $options['where'] ) ? $options['where'] : "" ).$this->parseOrder( isset( $options['order'] ) ? $options['order'] : "" ).$this->parseLimit( isset( $options['limit'] ) ? $options['limit'] : "" );
				if ( stripos( $sql, "where" ) === FALSE && $options['where'] !== TRUE )
				{
						return FALSE;
				}
		...
		public function parsePriority( $options )
		{
				if ( $Tmp_0 )
				{
						if ( in_array( isset( $options['priority'], $options['priority'] ), array( "LOW_PRIORITY", "QUICK", "IGNORE", "DELAYED", "HIGH_PRIORITY", "SQL_CACHE", "SQL_NO_CACHE" ) ) )
						{
		...
		public function insert( $data, $options = array( ), $replace = FALSE )
		{
				...
				$sql = ( $replace ? "REPLACE " : "INSERT " ).$this->parsePriority( $options )." INTO ".$this->parseTable( $options )." (".implode( ",", $fields ).") VALUES (".implode( ",", $values ).")";
				return DB::execute( $sql );
		}
