...
class Model
{
		...
		{
				if ( in_array( strtolower( $method ), array( "table", "order", "where", "on", "limit", "having", "group", "lock", "distinct" ), TRUE ) )
				{
				...
				if ( in_array( strtolower( $method ), array( "page" ), TRUE ) )
				{
						...
						else if ( !is_numeric( $args[0] ) && $args[0] <= 0 )
						{
								$args[0] = 10;
						}
						$this->options[strtolower( $method )] = $args[0];
		...
		public function select( $options = array( ) )
		{
				...
				if ( empty( $options['where'] ) && !is_numeric( $options['limit'] ) )
				{
						$options['limit'] = 30;
				}
				else if ( $options['where'] !== TRUE && !is_numeric( $options['limit'] ) )
				{
						$options['limit'] = 1000;
				}
