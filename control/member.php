...
class memberControl extends BaseMemberControl{
	...
	public function orderOp() {
		...
		$array['order_state']	= (in_array(trim($_GET['state_type']),array('order_pay','order_submit','order_no_shipping','order_shipping','order_finish','order_cancal','order_refer','order_confirm')) ? trim($_GET['state_type']) : '');
		...
		if(is_array($order_list) && !empty($order_list)) {
			...
			foreach ($order_list as $v) {
				$order_id_string	.= intval($v['order_id']).',';
				if(!in_array($v['store_id'],$store_id_array)) $store_id_array[] = $v['store_id'];
			}
			$order_id_string	= substr($order_id_string,0,-1);
			$order_list		= array();
			$order_list		= $model_my_order->myOrderGoodsList(array('order_id_string'=>$order_id_string));
...
//			}
//		}else{
//			$timeout = stream_context_create(array('http' => array('timeout' => 5))); 	//5秒超时
//			$content = @file_get_contents($url,false,$timeout);
//		}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'address':
				$menu_array = array(
				1=>array('menu_key'=>'address','menu_name'=>$lang['nc_member_path_address_list'],	'menu_url'=>'index.php?act=member&op=address'));
				break;
			case 'member_order':
				$menu_array = array(
				1=>array('menu_key'=>'member_order','menu_name'=>$lang['nc_member_path_order_list'],	'menu_url'=>'index.php?act=member&op=order'));
				break;
		}
