...
class show_groupbuyControl extends BaseHomeControl {
            ...
            if($current < $start_time) {
                ...
                if($current > $end_time) {
                    ...
                    if($groupbuy_info['def_quantity'] >= $groupbuy_info['max_num']) {
                        ...
                        if($groupbuy_info['def_quantity'] > $groupbuy_info['min_quantity']) {
                            $state_message['text'] = Language::get('groupbuy_message_success');
                        } else {
	 ...
	 * 购物流程第三步支付
	 */
	public function groupbuy_pay($step3_arr) {
		//是否可以使用货到付款
        $is_cod = true;
		$this->getPayment($step3_arr['store_id']);
		Tpl::output('order_id',$step3_arr['order_id']);
		Tpl::output('order_sn',$step3_arr['order_sn']);
		
		//查询订单信息
		$model_order= Model('order');
		$order_info	= $model_order->getOrderById($step3_arr['store_id']);
        ...
        Tpl::output('is_cod',$is_cod);
		Tpl::showpage('groupbuy_pay');
	}
   	...
   	public function getPayment($store_id) {
   		if (!C('payment')){
			...
			$online_array	= $model_payment->listStorePayment(1,$store_id);
			$offline_array = $model_payment->listStorePayment(0,$store_id);
   		}
//   		var_dump($offline_array);exit;
