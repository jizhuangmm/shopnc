...
//		$store_grade = $model_grade->getOneGrade(intval($_POST['grade_id']));
		...
		if (chksubmit()){
			...
			$shop_array['store_audit']	= ($store_grade['sg_confirm'] == 1 ? 0 : 1 );
			...
			if($state) {
				...
				//插入店铺缓存表
				$model->table('cache_store')->insert(array('store_id'=>$state));

				showDialog(Language::get('store_save_create_success'),'index.php?act=store','succ');
			} else {
