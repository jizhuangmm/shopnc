    ...
    public function templateaddOp(){
        ...
        if(chksubmit()){
	        ...
	        if ($error){
	        	...
	        	$insert_arr['voucher_t_eachlimit'] = intval($_POST['eachlimit'])>0?intval($_POST['eachlimit']):1;
	        	//自定义图片
    ...
    public function templateeditOp(){
        ...
        if(chksubmit()){
	        ...
	        if ($error){
	        	...
	        	$update_arr['voucher_t_eachlimit'] = intval($_POST['eachlimit'])>0?intval($_POST['eachlimit']):1;
	        	//自定义图片
		        if (!empty($_FILES['customimg']['name'])){
		        	$upload = new UploadFile();
		        	$upload->set('default_dir',	ATTACH_VOUCHER.DS.$_SESSION['store_id']);
