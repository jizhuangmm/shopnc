...
//		$step3_arr['order_id'] = $order_id;
	...
	private function getLegalGoods($spec_id,$quantity){
		...
		if ($goods_info['store_id'] == $_SESSION['store_id']){
			showMessage(Language::get('cart_add_cannot_buy'),"index.php?act=goods&goods_id={$goods_info['goods_id']}&id={$goods_info['store_id']}",'','error');
		}
		//查询店铺的基本信息
		$store_id = $goods_info['store_id'];
		$model_store = Model('store');
        $store_info = $model_store->shopStore(array('store_id'=>"$store_id",'store_state'=>'1','store_audit'=>'1'));//店铺开启并通过审核
		if (empty($store_info)){
		...
		if ($quantity > $goods_info['spec_goods_storage']){
			$quantity = $goods_info['spec_goods_storage'];
		}
	...
	private function getLegalBundlingGoods($spec,$quantity,$bl_id){
		...
		$store_info = $model->table('store')->where('store_id='.$store_id.' and store_state=1 and store_audit=1')->find();//店铺开启并通过审核
		if (empty($store_info)){
			showMessage(Language::get('cart_index_not_exists_store'),'index.php','','error');
		}
	...
	public function step2Op() {
		...
		$step3_arr['order'] = $output_order;
		$step3_arr['goods_name'] = $output_goods_name;		
//		$step3_arr['store_id'] = $legalGoods_info['store_info']['store_id'];
//		$step3_arr['store_memberid'] = $legalGoods_info['store_info']['member_id'];
//		$step3_arr['order_amount'] = $order_amount;
//		$step3_arr['order_sn'] = $order_sn;
//		$step3_arr['order_id'] = $order_id;
        $this->step3($step3_arr);
