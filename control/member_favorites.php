...
class member_favoritesControl extends BaseMemberControl{
	...
	public function fglistOp(){
		...
		if (!empty($favorites_list) && is_array($favorites_list)){
			...
			'goods.goods_id,goods.goods_name,goods.store_id,goods.goods_image,goods.goods_store_price,goods.commentnum,goods.salenum,goods.goods_collect,store.store_name,store.member_id,store.member_name,store.store_qq,store.store_ww,store.store_msn','store');
	...
	public function fslistOp(){
		...
		if (!empty($favorites_list) && is_array($favorites_list)){
			...
			$store_list = $store_model->getStoreGoods($store_list,$condition);//30天内新上架的商品数
			if (!empty($store_list) && is_array($store_list)){
