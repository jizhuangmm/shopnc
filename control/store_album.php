...
//		Tpl::output('show_page',$page->show());
	...
	public function pic_listOp(){
		 ...
		 */
		$page	= new Page();
        if(in_array($_GET['item'] , array('groupbuy', 'bundling_add', 'bundling_add_desc'))) {
            $page->setEachNum(12);
        } else {
            $page->setEachNum(16);
        }
	...
	public function replace_image_uploadOp() {
		...
		if($_POST['form_submit'] == 'ok'){
			...
			$pic_name	= array_pop(explode(DS, $apic_info['apic_cover']));			//文件名称
			/**
		 	 * 上传图片
		 	 */
			$upload = new UploadFile();
	...
	public function ajax_change_imgmessageOp(){
		$str = array_pop(explode('/', $_GET['url']));
		$str = explode('_', $str);
		/**
		 * 实例化图片模型
		 */
