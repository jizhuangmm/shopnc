...
class indexControl extends BaseHomeControl{
	...
	public function indexOp(){
		...
		$r_store = $this->_store();//推荐店铺
		Tpl::output('show_recommend_store',$r_store);
		$f_store = $this->_favorites_store();//收藏店铺
		Tpl::output('show_favorites_store',$f_store);
		$n_store = $this->_new_store();//最近加盟店铺
	...
	private function _store(){
		if (!$list = F('store','','cache/index')){
			$model_index = Model('index');
			$list = $model_index->getRecommendStore(3);
		}
		F('store',$list,'cache/index');
		return $list;
	}
	
	//收藏店铺
	private function _favorites_store(){
		if (!$list = F('favorites_store','','cache/index')){
			$model_index = Model('index');
			$list = $model_index->getFavoritesStore(3);
		}
		F('favorites_store',$list,'cache/index');
		return $list;
	}
	
	//最近加盟店铺
	private function _new_store(){
		if (!$list = F('new_store','','cache/index')){
			$model_index = Model('index');
			$list = $model_index->getNewStore(3);
		}
		F('new_store',$list,'cache/index');
		return $list;
	}
	...
	private function _promotion(){
		...
		if (!file_exists($cache_file) || filemtime($cache_file) <= (time()-SESSION_EXPIRE)){//文件不存在或过期时重新生成
			...
			$model = Model();
			$list = $model->table('p_xianshi_goods,p_xianshi')->field($field)->on('p_xianshi_goods.xianshi_id=p_xianshi.xianshi_id')->where($condition)->order('rand()')->limit($limit)->select();//防止出现全是一个店铺的使用随机排序
