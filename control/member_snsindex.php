...
class member_snsindexControl extends BaseMemberControl {
	...
	public function addtraceOp(){
		...
		$validate_arr[] = array("input"=>$_POST["content"], "validator"=>'Length',"min"=>0,"max"=>150,"message"=>Language::get('sns_content_beyond'));
		//发帖数超过最大次数出现验证码
	...
	public function sharegoodsOp(){
		if ($_POST['form_submit'] == 'ok'){
			...
			$validate_arr[] = array("input"=>$_POST["comment"], "validator"=>'Length',"min"=>0,"max"=>150,"message"=>Language::get('sns_content_beyond'));
			//发帖数超过最大次数出现验证码
			if(intval(cookie('weibonum'))>=self::MAX_RECORDNUM){
				$validate_arr[] = array("input"=>$_POST["captcha"], "require"=>"true","message"=>Language::get('wrong_null'));
			}
			...
			if ($result){
				...
				$content_str .= "<div class=\"fd-media\">
					<div class=\"goodsimg\"><a target=\"_blank\" href=\"%siteurl%".ncUrl(array('act'=>'goods','goods_id'=>$goods_info['goods_id'],'id'=>$goods_info['store_id']), 'goods')."\"><img src=\"".thumb($goods_info,'small')."\" onload=\"javascript:DrawImage(this,120,120);\" alt=\"{$goods_info['goods_name']}\"></a></div>
					<div class=\"goodsinfo\">
						<dl>
							<dt><a target=\"_blank\" href=\"%siteurl%".ncUrl(array('act'=>'goods','goods_id'=>$goods_info['goods_id'],'id'=>$goods_info['store_id']), 'goods')."\">".$goods_info['goods_name']."</a></dt>
				...
				if (C('share_isuse') == 1){
					...
					$params['url'] = SiteUrl.DS.ncUrl(array('act'=>'goods','goods_id'=>$goods_info['goods_id'],'id'=>$goods_info['store_id']), 'goods');
	...
	public function deltraceOp(){
		...
		if ($result){
			...
			if ($_GET['type'] == 'href'){
				...
				$js = "$('[nc_type=\"tracerow_{$id}\"]').remove();";
	...
	public function editprivacyOp(){
		...
		if ($result){
			...
			$js = "var obj = $(\"#recordone_{$id}\").find(\"[nc_type='privacytab']\"); $(obj).find('span').removeClass('selected');$(obj).find('ul li:nth-child(".$privacy_item.")').find('span').addClass('selected');";
	...
	public function delgoodsOp(){
		...
		if ($result){
			...
			$js = "var obj = $(\"#recordone_{$id}\").remove();";
			showDialog(Language::get('nc_common_del_succ'),'','succ',$js);
		}else {
			showDialog(Language::get('nc_common_del_fail'),'','error');
		}
	...
	public function addcommentOp(){
		...
		$validate_arr[] = array("input"=>$_POST["commentcontent"], "validator"=>'Length',"min"=>0,"max"=>150,"message"=>Language::get('sns_content_beyond'));
		//评论数超过最大次数出现验证码
	...
	public function editlikeOp(){
		...
		if ($result){
			...
			$insert_arr['trace_copycount'] = 0;
			$result = $tracelog_model->tracelogAdd($insert_arr);
			$js = "var obj = $(\"#likestat_{$goods_info['goods_id']}\"); $(obj).find('i').addClass('noaction');$(obj).find('a').addClass('noaction'); var countobj=$('[nc_type=\'likecount_{$goods_info['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
	...
	public function addforwardOp(){
		...
		$validate_arr[] = array("input"=>$_POST["forwardcontent"], "validator"=>'Length',"min"=>0,"max"=>150,"message"=>Language::get('sns_content_beyond'));
		//发帖数超过最大次数出现验证码
	...
	public function sharegoods_oneOp(){
		...
		$goods_info['goods_url'] = ncUrl(array('act'=>'goods','goods_id'=>$goods_info['goods_id'],'id'=>$goods_info['store_id']), 'goods');
		//判断系统是否开启站外分享功能
		if (C('share_isuse') == 1){
			//站外分享接口
			$model = Model('sns_binding');
