...
class show_storeControl extends BaseStoreControl {
	...
	public function indexOp(){
		...
		//检查店铺状态
		$store_info = $this->checkStoreState($store_id,Language::get('show_store_index_store_not_exists'));
		//店铺是否为空
		if (empty($store_info)){
			showMessage(Language::get('show_store_index_store_not_exists'),'','html','error');
		}
		...
		//得到店铺展示信息
		$this->show_store_info($store_info);
		...
		$recommended_goods_list = $goods_class->getGoods(array(
			...
			'goods_show'=>1,
			'goods_commend'=>1,
			'order'=>'`goods`.goods_add_time desc',
			'limit'=>'0,12'
		),'','goods.*','goods');
	...
	public function creditOp(){
		...
		//检查店铺状态
		$store_info = $this->checkStoreState($id);
		//得到店铺展示信息
		$store_info = $this->show_store_info($store_info);
		//查询店铺动态评价
	...
	public function goods_allOp(){
		 ...
		 * 检查店铺状态
		 */
		$store_info = $this->checkStoreState($store_id,$lang['show_store_index_store_not_exists']);
		/**
		 * 检查
		 */
		if (empty($store_info)){
		...
		switch (trim($_GET['key'])){
			...
			case 'collect':
				...
				break;
			case 'new':
				$conditionArr['order'] = 'goods.goods_add_time '.$_GET['order'];
				break;
			default:
				$conditionArr['order'] = 'goods.goods_add_time desc,goods.goods_id desc';
				break;
		}
		//查询分类下的子分类
		if ($_GET['stc_id']){
