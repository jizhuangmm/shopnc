...
class storeControl extends BaseMemberStoreControl {
	...
	public function statisticsOp() {
		...
		$condition = array();
		$condition['store_id'] = $_SESSION['store_id'];
		$condition['order_state'] = array(array('gt',0),array('lt',20),'and');
		...
		$evalseller = $model->table('order')->where(array('store_id'=>$_SESSION['store_id'],'order_state'=>40))->count();
		...
		if($_GET['type'] == 'ok') {
			...
			if ($_POST['form_submit'] == 'ok'){
				if (!empty($_FILES['csv'])){
						...
						switch (strtoupper($_POST['charset'])){
							...
							case 'BIG5':
								if (strtoupper(CHARSET) !== 'BIG5'){
									$data = iconv('BIG5',strtoupper(CHARSET),$data);
								}
								break;
						}
						if (!empty($data)){
							 ...
							 */
							$tmp_array = array();
							$tmp_array = explode(',',trim(trim($data),','));
							/**
							...
							$insert_array = array();
							$insert_array['stc_sort'] = trim($tmp_array[0]);
							$insert_array['stc_parent_id'] = !empty($$tmp_deep) ? trim($$tmp_deep) : 0 ;
							$insert_array['stc_name'] = trim($tmp_array[count($tmp_array)-1]);
							$insert_array['stc_name'] = str_replace('__NC__', ',', trim($tmp_array[count($tmp_array)-1]));
							$gc_id = $model_class->importGoodsClass($insert_array);
							/**
							 * 赋值这个深度父ID
							 */
					...
					@header("Location: index.php?act=store&op=store_goods_class");
					exit;
				}else {
					showMessage(Language::get('store_goods_class_csc_null'),'','html','error');
				}
			}
			/**
			 * 输出模版
			 */
			Tpl::showpage('store_goods_class_import','null_layout');
			...
			if (!empty($_POST['form_submit']) && $_POST['form_submit'] == 'ok'){
				 ...
				 * 分类信息
				 */
				$class_list = $model_class->getTreeClassList(array('store_id'=>$_SESSION['store_id']));

				@header("Content-type: application/unknown");
				@header("Content-Disposition: attachment; filename=store_goods_class.csv");
				if (!empty($class_list) && is_array($class_list)){
					foreach ($class_list as $k => $v){
						 ...
						 * 分类名称
						 */
						$tmp['stc_name'] = $v['stc_name'];
						$tmp['stc_name'] = str_replace(',', '__NC__', $v['stc_name']);
						/**
						 * 转码 utf-gbk
						 */
						if (strtoupper(CHARSET) == 'UTF-8'){
			 ...
			 * 输出模版
			 */
			Tpl::showpage('store_goods_class_export','null_layout');
		} else {
	...
	public function store_orderOp() {
		...
		if (is_array($goods_list) && !empty($goods_list)){
			...
			foreach ($goods_list as $val) {
				...
				$val['state_info']		= orderStateInfo($val['order_state']);
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			...
			case 'store_setting':
				...
				2=>array('menu_key'=>'store_certified','menu_name'=>Language::get('nc_member_path_store_cert'),'menu_url'=>'index.php?act=store&op=store_certified'),
				3=>array('menu_key'=>'store_map','menu_name'=>Language::get('nc_member_path_store_map'),'menu_url'=>'index.php?act=map'),
				4=>array('menu_key'=>'store_slide','menu_name'=>Language::get('nc_member_path_store_slide'),'menu_url'=>'index.php?act=store&op=store_slide'),
				5=>array('menu_key'=>'store_printsetup','menu_name'=>Language::get('nc_member_path_store_printsetup'),'menu_url'=>'index.php?act=store&op=store_printsetup')
				);
				break;
			case 'store_theme':
				$menu_array = array(
