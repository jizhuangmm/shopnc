		...
		if($listgoods){
			foreach($listgoods as $replace_key => $replace_val){
				...
				if($replace_val['goods_image']){
					$listgoods[$replace_key]['image_url']	= ATTACH_PATH.'/flea/goods/'.$replace_val['member_id'].'/'.$replace_val['goods_image'];
				}else{
					$listgoods[$replace_key]['image_url']	= TEMPLATES_PATH.'/images/default_goods_image.gif';
				}
				$exge='/<[^>]*>|\s+/';
