...
class homeControl extends BaseMemberControl {
	...
	public function memberOp() {
		...
		if (chksubmit()){
			...
			$member_array['member_msn']			= $_POST['member_msn'];
	...
	public function messageOp() {
		...
		//查询新接收到卖家的消息
		$newseller = $this->receivedSellerNewNum();
		Tpl::output('newseller',$newseller);
		//查询会员是否允许发送站内信
	 ...
	 * @return int
	 */
	private function receivedSellerNewNum(){
		$message_model = Model('message');
		$condition_arr = array();
		$condition_arr['message_type'] = '2';//卖家消息
		$condition_arr['to_member_id'] = $_SESSION['member_id'];
		$condition_arr['no_del_member_id'] = $_SESSION['member_id'];
		$condition_arr['no_read_member_id'] = $_SESSION['member_id'];
		$countnum = $message_model->countMessage($condition_arr);
	...
	public function dropcommonmsgOp() {
		...
		$drop_type = trim($_GET['drop_type']);
		if(!in_array($drop_type,array('msg_private','msg_list')) || empty($message_id)) {
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'member':
				...
				2=>array('menu_key'=>'passwd',	'menu_name'=>Language::get('home_member_modify_password'),'menu_url'=>'index.php?act=home&op=passwd'),
				3=>array('menu_key'=>'email',	'menu_name'=>Language::get('home_member_modify_email'),'menu_url'=>'index.php?act=home&op=email'),
				4=>array('menu_key'=>'avatar',	'menu_name'=>Language::get('home_member_modify_avatar'),'menu_url'=>'index.php?act=home&op=avatar'));
				break;
			case 'message':
				$menu_array	= array(
				...
				//4=>array('menu_key'=>'seller',	'menu_name'=>Language::get('home_message_seller_message'),	'menu_url'=>'index.php?act=home&op=sellermsg'),
				);
				if($menu_key == 'sendmsg') {
