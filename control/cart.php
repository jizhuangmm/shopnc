...
//        $mansong['mansong_flag'] = $mansong_flag;
		...
		if ($_SESSION['member_id']){
			...
			if($drop_state) {
				...
				if ($quantity >0){
					...
					echo json_encode(array('done'=>'true','amount'=>ncPriceFormat($amount),'store_amount'=>ncPriceFormat($store_amount),'quantity'=>$quantity,'store_quantity'=>$store_quantity));
				}else {
					echo json_encode(array('done'=>'true','amount'=>ncPriceFormat(0),'store_amount'=>ncPriceFormat(0),'quantity'=>0,'store_quantity'=>0));
				}
			} else {
				echo json_encode(array('msg'=>Language::get('cart_drop_del_fail','UTF-8')));
			}
		}else{
			//未登录删除cookie购物车信息
			$cart_arr = array();
			...
			if (cookie('cart')){
				...
				if (!empty($cart_arr)){
					...
					if (!empty($cart_arr)){
						//cookie购物车存在
						echo json_encode(array('done'=>'true','amount'=>ncPriceFormat($all_price),'quantity'=>count($cart_arr)));
						setNcCookie('cart',serialize($cart_arr),90*24*3600);
					}else{
						//cookie购物车不存在
						echo json_encode(array('done'=>'true','amount'=>ncPriceFormat(0),'quantity'=>0));
						setNcCookie('cart','',-1);
					}
				}else {
					echo json_encode(array('msg'=>Language::get('cart_drop_del_fail','UTF-8')));
	...
	private function getLegalGoods($store_id){
		...
		foreach ($goods_list as $k => $v) {
			...
			if (intval($v['spec_goods_storage']) < $quantity){
				$quantity = intval($v['spec_goods_storage']);
			}
	...
	public function step2Op() {
		...
		$step3_arr['shipping_info'] = $shipping_info;
		$step3_arr['address_info'] = $address_info;
		$step3_arr['order'] = $output_order;
		$step3_arr['goods_name'] = $output_goods_name;
//		$step3_arr['store_id'] = $store_id;
//		$step3_arr['store_memberid'] = $legalGoods_list['store_info']['member_id'];
//		$step3_arr['order_amount'] = $order_amount;
//		$step3_arr['order_sn'] = $order_sn;
//		$step3_arr['order_id'] = $order_id;
        $this->step3($step3_arr);
	}
	/**
	 * 购物流程第三步支付
	 */
	public function step3($step3_arr) {
   		if (!C('payment')){
	   		//取得平台支付方式
			...
			$online_array	= $model_payment->listStorePayment(1,$step3_arr['order']['store_id']);
			$offline_array = $model_payment->listStorePayment(0,$step3_arr['order']['store_id']);
   		}
		Tpl::output('online_array',$online_array);
		Tpl::output('offine_array',$offline_array);
		Tpl::output('order',$step3_arr['order']);
		Tpl::output('goods_name',$step3_arr['goods_name']);
//		Tpl::output('order_id',$step3_arr['order_id']);
//		Tpl::output('order_sn',$step3_arr['order_sn']);
//		Tpl::output('store_memberid',$step3_arr['store_memberid']);
//		Tpl::output('order_amount',ncPriceFormat($step3_arr['order_amount']));
...
//		Tpl::output('store_memberid',$store_info['member_id']);
//		Tpl::output('order_amount',$order_info['order_amount']);
		Tpl::showpage('cart_step2');
	} 
     /**
      * 添加新的收货地址
