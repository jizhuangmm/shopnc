...
class goodsControl extends BaseStoreControl {
	...
	public function indexOp() {
		...
		//检查店铺状态
		$store_info = $this->checkStoreState($goods_array['store_id'],Language::get('goods_index_goods_store_closed'));
		...
		if (empty($_times)){
			$model_store_goods->updateGoods(array('goods_click'=>array('sign'=>'increase','value'=>'1')),$goods_id);
		...
		$goods_commend_list = $model_store_goods->getGoods(array(
			...
			'goods_show'=>1,
			'goods_commend'=>1,
			'order'=>'goods_commend desc,goods_add_time desc',
			'limit'=>'0,4'
