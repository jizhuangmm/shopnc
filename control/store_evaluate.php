...
class store_evaluateControl extends BaseMemberStoreControl{
    ...
    public function addOp(){
		...
		//订单为'已收货'状态，并且未评论
		$order_info = $order_model->getOrderById($order_id,'simple',array('seller_id'=>"{$_SESSION['member_id']}",'evalseller_status'=>'0','order_state'=>'order_finish'));
    	if (empty($order_info)){
		...
		if (!$_POST){
			...
			//处理商品评价
			//根据订单评价状态判断互评状态(如果订单买家已评价卖家，则互评状态为2否则为1)
			...
			foreach ($_POST['goods'] as $k=>$v){
				if (intval($v['points'])>0){//判断该商品是否点击了评论
					$insert_arr = array();
