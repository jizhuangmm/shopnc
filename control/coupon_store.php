	...
	public function detailOp(){	
		...
		if(is_array($coupon_detail)&&!empty($coupon_detail)){
		   /**
		 	* 检查店铺状态
			*/
			$store_info = $this->checkStoreState($coupon_detail['store_id'],Language::get('goods_index_goods_store_closed'));
			/**
			 * 检查
			 */
			if (empty($store_info)){
				showMessage(Language::get('coupon_index_no_store'),'','html','error');//'您要查看的店铺不存在'
			}
