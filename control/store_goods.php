...
class store_goodsControl extends BaseMemberStoreControl {
	public function __construct() {
		...
		if(intval($store_grade[0]['store_audit']) == 0) {//卖家店铺是否审核的判断
	...
	public function save_goodsOp() {
		...
		if (chksubmit()){
			...
			$goods_array['goods_indate']			= $_POST['goods_indate'];
			...
			switch ($_POST['goods_show']){
				...
				case 1:
					...
					$goods_array['goods_endtime']	= time()+intval($_POST['goods_indate'])*86400;
					break;
				case 2:
					$goods_array['goods_show']		= 0;
					$goods_array['goods_starttime']	= strtotime($_POST['starttime']) + intval($_POST['starttime_H'])*3600 + intval($_POST['starttime_i'])*60;
					$goods_array['goods_endtime']	= strtotime($_POST['starttime']) + intval($_POST['starttime_H'])*3600 + intval($_POST['starttime_i'])*60 + intval($_POST['goods_indate'])*86400;
	...
	public function edit_save_goodsOp() {
		...
		if (chksubmit() &&  $goods_id!= 0){
			...
			if($state) {
				...
				redirect(ncUrl(array('act'=>'goods','goods_id'=>$goods_id,'id'=>$_SESSION['store_id']), 'goods'));
//				showMessage($lang['store_goods_index_goods_edit_success'],ncUrl(array('act'=>'goods','goods_id'=>$goods_id,'id'=>$_SESSION['store_id']), 'goods'));
			} else {
	...
	public function goods_showOp(){
         ...
         * 检测商品是否存在
         */
        $goods_array = $model_store_goods->getGoods(array('goods_id'=>$goods_id), '', 'goods_indate', 'goods');
        if(!is_array($goods_array) || empty($goods_array)){
        	showMessage(Language::get('para_error'),'','html','error');
        }
		$state	= $model_store_goods->updateGoods(array('goods_show'=>'1','goods_starttime'=>time(),'goods_endtime'=>(time()+86400*intval($goods_array[0]['goods_indate']))), $goods_id);
		if($state) {
	...
	public function image_uploadOp() {
		$lang	= Language::getLangContent();
			...
			$param = array();
			$param['field']	= array('store_id','is_default');
			$param['value']	= array($_SESSION['store_id'],'1');
			 ...
			 * 取得图像大小
			 */
			list($width, $height, $type, $attr) = getimagesize(BasePath.DS.'upload'.DS.'store'.DS.'goods'.DS.$_SESSION['store_id'].DS.$img_path);
	...
	public function ajax_class_searchOp() {
		...
		if(is_array($class_tag_array)){
			foreach ($class_tag_array as $v){
				...
				if(in_array(trim($_GET['value']), $tag_name_array)){
		...
		if(!$_POST){
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					...
					$param['goods_form']			= '1';
					$param['goods_indate']			= $record['goods_indate'];
					$param['goods_starttime']		= time();
					$param['goods_endtime']			= time()+intval($record['goods_indate'])*86400;
