...
class store_gradeModel{
	...
	public function setGradeCache(){
		$storeGradeList = $this->getGradeList();
		$cacheArr = array();
		if(is_array($storeGradeList) && count($storeGradeList)>0){
			foreach ($storeGradeList as $v){
				$cacheArr[$v['sg_id']]['name'] = $v['sg_name'];
				$cacheArr[$v['sg_id']]['sort'] = $v['sg_sort'];
			}
		}
		F('store_grade',$cacheArr);
	}
}