...
class my_orderModel extends Model {
	...
	public function myOrderGoodsList($param,$page = '') {
		...
		//得到条件语句
		$condition_str	= $this->getCondition($param);
		$array['table']		= 'order,order_goods,payment';
		$array['field']		= '`order`.*,`order_goods`.*,`payment`.payment_id,`payment`.payment_code,`payment`.payment_name,`payment`.payment_info,`payment`.payment_config,`payment`.payment_online,`payment`.payment_state,`payment`.payment_sort';
		$array['join_type']	= 'LEFT JOIN';
		$array['join_on']	= array('order.order_id=order_goods.order_id','order.payment_id=payment.payment_id');
	...
	private function getCondition($condition_array){
		...
		if($condition_array['order_state'] == 'order_pay') {
			$condition_sql	.= ' and order_state>0 and order_state<20';
		}
		...
		if($condition_array['order_evalbuyer_able'] != '') {
			$condition_sql	.= " and order.order_state=40 and order.evaluation_status = 0 and (((order.finnshed_time+60*60*24*15)>".time().") or ((order.evalseller_time+60*60*24*15)>".time().")) ";
		}
