...
class orderModel{
	...
	public function addLogOrder($order_step,$order_id,$message='',$lang=array()) {
		...
		if (strtoupper(CHARSET) == 'GBK'){
			$log_array['order_state'] = Language::getGBK($log_array['order_state']);
			$log_array['change_state'] = Language::getGBK($log_array['change_state']);
		}
		*/
		//return Db::insert('order_log',$log_array);
	...
	private function getCondition($condition_array){
		...
		if($condition_array['order_evalseller_able'] != '') {
			$condition_sql	.= " and order.order_state=40 and order.evalseller_status = 0 and (((order.finnshed_time+60*60*24*15)>".time().") or ((order.evaluation_time+60*60*24*15)>".time().")) ";
		}
